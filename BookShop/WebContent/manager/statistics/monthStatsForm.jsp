...
<head>
	...
	<title>월별 판매리스트(꺾은선)</title>
</head>
<body>

	<div class="container">
		<h2 align="center"><b>월별 판매리스트(꺾은선)</b></h2><hr><br>
		<form class="form-horizontal" role="form" method="post" name="monthStatsForm" action="monthStatsForm.jsp">
			<div class="form-group">
				...
				<div class="col-sm-2">
					<input type="submit" class="btn btn-danger btn-sm" value="검색하기" action="javascript:window.location=monthStatsForm.jsp">
	...
	<script>
		...
		new Morris.Line({
			element: 'mychart',
			data: [
					{month: m01, value: <%=buyMonthList.getMonth01()%>},
					{month: m02, value: <%=buyMonthList.getMonth02()%>},
					{month: m03, value: <%=buyMonthList.getMonth03()%>},
					{month: m04, value: <%=buyMonthList.getMonth04()%>},
					{month: m05, value: <%=buyMonthList.getMonth05()%>},
					{month: m06, value: <%=buyMonthList.getMonth06()%>},
					{month: m07, value: <%=buyMonthList.getMonth07()%>},
					{month: m08, value: <%=buyMonthList.getMonth08()%>},
					{month: m09, value: <%=buyMonthList.getMonth09()%>},
					{month: m10, value: <%=buyMonthList.getMonth10()%>},
					{month: m11, value: <%=buyMonthList.getMonth11()%>},
					{month: m12, value: <%=buyMonthList.getMonth12()%>},
			      ],
			xkey: 'month', //그래프 데이터에서 x축에 해당하는 값의 이름
			ykeys: ['value'], //그래프 데이터에서 y축에 해당하는 값의 이름
			labels: ['판매권수'] //데이터의 값을 표현
		});
	</script>
