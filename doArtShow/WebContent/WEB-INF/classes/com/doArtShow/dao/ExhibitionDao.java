...
public class ExhibitionDao {
	...
	public ArrayList<ExhibitionDto> searchExhibition(String search) {
		...
		try {
			...
			sql  = 	" SELECT ExhID, ExhPlaceAddr1, ExhName , ArtistName , ExhPlace,  ExhStartDate , ExhEndDate , ImageFile1 FROM  artshow WHERE ( ExhName like ? OR ArtistName like ? OR ExhPlace like ? ) AND ActiveFlag='Y' GROUP BY ExhID ORDER BY ExhEndDate DESC ";
	...
	public ArrayList<ExhibitionDto> searchMapExhibition() {
		...
		try {
			conn = ds.getConnection();
			sql  = 	" SELECT ExhID, ExhPlaceAddr1, ExhName , ArtistName , ExhPlace,  ExhStartDate , ExhEndDate , ImageFile1 FROM  artshow WHERE ActiveFlag='Y' GROUP BY ExhID ORDER BY ExhEndDate DESC ";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	//----------------------------------seran----------------------------------
		...
		public List<ExhListDto> selectList(){
			...
			String sql = "SELECT exhID, imageFile1, exhName, exhPlace, exhStartDate, exhEndDate FROM  artshow WHERE ActiveFlag!='N' ORDER BY registerDate DESC";
		...
		public int getListCount(){
			...
			int listCnt = 0;
			String sql = "SELECT count(*) FROM  artshow WHERE ActiveFlag!='N'";
		...
		public int getRevCount(int exhID){
			...
			int RevCnt = 0;
			String sql = "SELECT count(*) FROM  review WHERE exhID=?";
		...
		public int wishCheck(String email, int exhID) {
			...
			try {
				conn = ds.getConnection();
				sql	= "SELECT wishCheck FROM  wishart WHERE email=? AND exhID=?";
				pstmt = conn.prepareStatement(sql);
...
// 메인페이지에서 슬라이드에 전시 리스트 보냄.
	public ArrayList<ExhibitionDto> indexExhibition() {
		...
		try {
			...
			dto=null;
			// 인기 조회순 9개
			sql = " SELECT ExhID, ExhPlaceAddr1, ExhName , ArtistName , ExhPlace,  ExhStartDate , ExhEndDate , ImageFile1 FROM  artshow  where activeflag='Y' ORDER BY ExhReadCount DESC LIMIT 9 ";
			pstmt=conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
				dto = new ExhibitionDto();
