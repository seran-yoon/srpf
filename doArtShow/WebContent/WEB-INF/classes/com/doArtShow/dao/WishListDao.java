...
public class WishListDao {
	...
	public void updateWishCheck(String email, int exhID) {
		...
		try {
			conn = ds.getConnection();
			sql = "UPDATE  wishart SET wishCheck=wishCheck+1 WHERE email=? AND exhID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			pstmt.setInt(2, exhID);
	...
	public void deleteWishCheck(String email, int exhID) {
		...
		try {
			conn = ds.getConnection();
			sql = "UPDATE  wishart SET wishCheck=0 WHERE email=? AND exhID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			pstmt.setInt(2, exhID);
	...
	public void insertWish(WishListDto wishDto) {
		...
		try {
			conn = ds.getConnection();
			sql = "INSERT INTO  wishart(email, exhID, wishCheck) VALUES (?,?,?)";
			pstmt = conn.prepareStatement(sql);
	...
	public int WishRowCheck(String email, int exhID) {
		...
		try {
			conn = ds.getConnection();
			sql	= "SELECT count(*) FROM  wishart WHERE email=? AND exhID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			pstmt.setInt(2, exhID);
