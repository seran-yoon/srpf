...
public class VisitListDao {
	...
	public int visitCheck(String email, int exhID) {
		...
		try {
			...
			sql	= "SELECT visitCheck FROM  visitart WHERE email=? AND exhID=?";
	...
	public int reviewCheck(String email, int exhID) {
		...
		try {
			conn = ds.getConnection();
			sql	= "SELECT revCheck FROM  visitart WHERE email=? AND exhID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			pstmt.setInt(2, exhID);
	...
	public void insertVisit(VisitListDto visitDto) {
		...
		try {
			conn = ds.getConnection();
			sql = "INSERT INTO  visitart(email, exhID, revCheck) VALUES (?,?,?)";
			pstmt = conn.prepareStatement(sql);
	...
	public void updateVisitCheck(String email, int exhID) {
		...
		try {
			conn = ds.getConnection();
			sql = "UPDATE  visitart SET visitCheck=visitCheck+1 WHERE email=? AND exhID=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			pstmt.setInt(2, exhID);
